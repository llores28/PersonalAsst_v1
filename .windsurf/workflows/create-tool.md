...
## 2) Scaffold the tool
```
nexus scaffold <tool-name> --description "What it does" --format json
```
...
## 4) Register the tool
Add the registration snippet (from scaffold output) to `Nexus/nexus/cli/bs_cli.py`.

## 5) Test
```
nexus <tool-name> --format human
```

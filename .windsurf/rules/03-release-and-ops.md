...
After any large fix, feature, or improvement to `src/` code, **you MUST update** the following living docs to reflect the changes:
...
**When to trigger:** Any change that adds/removes/renames an agent, tool, command, API endpoint, env var, container, or DB model. Minor typo fixes or internal refactors that don't change behavior do not require doc updates.

**How:** At the end of the task, review each doc above and update only the sections affected by the change. Do not rewrite entire docs — make targeted edits.
...
8. Living docs updated (see Documentation Update Rule above).
